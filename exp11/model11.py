"""exp11/model11.py — PCVRFusionFormer11

Base: exp6/model6.py (PCVRFusionFormer6)
Changes:
  1. SIM (Search-based Interest Model):
       CrossDomainInterestTransfer target-aware pooling uses Hard Search
       to select top-K events (by dot-product with item_repr) before
       applying soft attention. sim_top_k=0 disables SIM (uses full L).
  2. PEPNet Gate:
       gate_bias_proj = Linear(d_model, 3) injects user context into gate.
       gates = sigmoid(gate_linear(gate_in) + gate_bias_proj(user_repr))

Alias:
  PCVRHyFormer = PCVRFusionFormer11  (for infer.py dynamic import)
"""
...
# ── CrossDomainInterestTransfer (SIM) ─────────────────────────────────────────

class CrossDomainInterestTransfer(nn.Module):
    """Two-step domain representation builder with SIM Hard Search.

    Step A: Target-aware pooling with SIM:
      - Hard Search: select top-K events by dot-product with item_repr
      - Soft Attention: apply softmax only over selected K events
      - Multiplicative combination with decay_weights (unchanged)
    Step B: Cross-domain MHA — domain representations attend to each other

    sim_top_k=0 disables SIM (full-L soft attention, original behavior).
    """
    ...
    def forward(
        ...
        for domain in domains:
            ...
            # Dot-product scores: item_repr as query over event sequence
            scores = torch.bmm(embs, item_repr.unsqueeze(-1)).squeeze(-1) / scale  # (B, L)

            if self.sim_top_k > 0:
                # Hard Search: select top-K valid events
                K = min(self.sim_top_k, L)
                scores_for_topk = scores.masked_fill(pad_mask, -1e9)
                _, topk_idx = scores_for_topk.topk(K, dim=-1)          # (B, K)

                # sim_mask: True = excluded from soft attention
                sim_mask = torch.ones(B, L, dtype=torch.bool, device=embs.device)
                sim_mask.scatter_(1, topk_idx, False)                   # False = selected

                # Soft attention over top-K only
                combined_mask = pad_mask | sim_mask
                attn_w = torch.softmax(
                    scores.masked_fill(combined_mask, -1e9), dim=-1
                )  # (B, L), near-zero outside top-K
            else:
                # SIM disabled: original full-L soft attention
                attn_w = torch.softmax(
                    scores.masked_fill(pad_mask, -1e9), dim=-1
                )  # (B, L)
...
class PCVRFusionFormer11(nn.Module):
    """PCVRFusionFormer11: exp6 base + SIM Hard Search + PEPNet Gate.

    SIM: CrossDomainInterestTransfer selects top-K events (hard search)
         before soft attention. sim_top_k=0 → original full-L behavior.
    PEPNet: gate_bias_proj(user_repr) shifts gate activations per-user,
            allowing user-context-aware signal weighting.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ── Cross-Domain Interest Transfer (SIM) ─────────────────────────────
        ...
        # Gate (B): LayerNorm before sigmoid + PEPNet user-context bias
        ...
        logging.info(
            f"PCVRFusionFormer11: U={self.num_user_ns} I={self.num_item_ns} "
            f"S={self.num_sequences} T={self.num_user_ns+self.num_item_ns+self.num_sequences} "
            f"d_model={d_model} blocks={num_hyformer_blocks} sim_top_k={sim_top_k} "
    ...
    def _score(self, all_tok: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Gate (B): LayerNorm + PEPNet user-context bias
    ...
    def reinit_high_cardinality_params(self, cardinality_threshold: int = 10000) -> "set[int]":
        ...
        for m in self.modules():
            if isinstance(m, nn.Embedding) and m.num_embeddings > cardinality_threshold + 1:
                ...
                count += 1
        logging.info(f"PCVRFusionFormer11: re-initialized {count} high-cardinality embeddings")
        return reinit_ptrs


# infer.py dynamic import requires PCVRHyFormer alias
PCVRHyFormer = PCVRFusionFormer11
