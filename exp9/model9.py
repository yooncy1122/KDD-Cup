"""exp9/model9.py — PCVRFusionFormer9

Changes from model6.py:
  seq_d: EventEncoder → SeqDStatisticalEncoder
    - Computes 47-dim statistical features from the full seq_d history
    - Linear(47 → d_model) + LayerNorm → (B, d_model)
    - No target-aware pooling; (B, d_model) fed directly to cross-domain MHA

  seq_a/b/c: EventEncoder unchanged.

  CrossDomainInterestTransfer: accepts optional precomputed_repr_dict to
    bypass pooling for any pre-computed domain representation.

  TB logging: entropy/target_attn_seq_d and lambda/seq_d are absent
    (StatisticalEncoder has neither attn_w nor log_lambda).

Alias:
  PCVRHyFormer = PCVRFusionFormer9  (for infer.py dynamic import)
"""
...
class SeqDStatisticalEncoder(nn.Module):
    ...
    def forward(
        ...
        # Padding mask: True where valid event
        ...
        parts.append((raw_len / 1000.0).unsqueeze(-1))   # (B, 1)

        # [1] log1p_len (normalized)
        parts.append((torch.log1p(raw_len) / 10.0).unsqueeze(-1))  # (B, 1)

        # [4] fid17 class distribution
        vals17 = seq_data[:, self.S_FID17, :].long()    # (B, L)
        dist17 = torch.stack(
        ...
        vals25 = seq_data[:, self.S_FID25, :].long()    # (B, L)
        dist25 = torch.stack(
        ...
        vals24 = seq_data[:, self.S_FID24, :].long()    # (B, L)
        dist24 = torch.stack(
        ...
        for thresh in self.RECENCY_BUCKETS:
            ...
            parts.append((rec / recency_denom).unsqueeze(-1))           # (B, 1) each
...
class PCVRFusionFormer9(nn.Module):
    """PCVRFusionFormer9: seq_d EventEncoder → SeqDStatisticalEncoder.

    seq_d uses 47-dim hand-crafted statistics projected to d_model via Linear.
    seq_a/b/c retain EventEncoder with target-aware pooling unchanged.
    CrossDomainInterestTransfer receives seq_d representation pre-computed,
    skipping the pooling step for seq_d while still including it in cross-domain MHA.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logging.info(
            f"PCVRFusionFormer9: U={self.num_user_ns} I={self.num_item_ns} "
    ...
    def _tokenize(self, inputs: ModelInput, training: bool) -> torch.Tensor:
        ...
        if self._tb_logs is not None:
            with torch.no_grad():
                ...
                # entropy: skip domains where attn_w is None (StatisticalEncoder)
    ...
    def reinit_high_cardinality_params(self, cardinality_threshold: int = 10000) -> "set[int]":
        ...
        for m in self.modules():
            if isinstance(m, nn.Embedding) and m.num_embeddings > cardinality_threshold + 1:
                ...
                count += 1
        logging.info(f"PCVRFusionFormer9: re-initialized {count} high-cardinality embeddings")
        return reinit_ptrs


# infer.py dynamic import requires PCVRHyFormer alias
PCVRHyFormer = PCVRFusionFormer9
