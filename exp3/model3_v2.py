"""exp3/model3_v2.py — PCVRFusionFormer3V2

DINEncoder scale fix over model3.py:
  - decay_score normalized: -lam * bucket / 63.0  → range [-1, 0]
  - attn_score normalized:  attn_mlp(inter) / sqrt(d_model)
  Both scores now operate in the same [-1, 0] / O(1) range so attn_mlp
  gradients are not swamped by the raw time-bucket magnitude.

Alias:
  PCVRHyFormer = PCVRFusionFormer3V2  (for infer.py dynamic import)
"""
...
class DINEncoder(nn.Module):
    ...
    def forward(
        ...
        # DIN attention score: MLP([q, k, q-k, q*k]), scaled to O(1)
        q = item_repr.unsqueeze(1).expand(-1, L, -1)         # (B, L, d_model)
        k = event_embs                                        # (B, L, d_model)
        inter = torch.cat([q, k, q - k, q * k], dim=-1)     # (B, L, 4*d_model)
        attn_score = self.attn_mlp(inter).squeeze(-1) / math.sqrt(event_embs.shape[-1])  # (B, L)

        # Time-decay score: normalized to [-1, 0] to match attn_score scale
        lam = torch.exp(self.log_lambda)
        decay = -lam * time_buckets.float() / 63.0           # (B, L), range [-1, 0]
...
class PCVRFusionFormer3V2(nn.Module):
    """PCVRFusionFormer3V2: PCVRFusionFormer3 with DINEncoder scale fix.

    attn_score / sqrt(d_model) and decay / 63.0 put both signals in the same
    magnitude range so attn_mlp receives meaningful gradients from the start.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logging.info(
            f"PCVRFusionFormer3V2: U={self.num_user_ns} I={self.num_item_ns} "
    ...
    def reinit_high_cardinality_params(self, cardinality_threshold: int = 10000) -> "set[int]":
        ...
        for m in self.modules():
            if isinstance(m, nn.Embedding) and m.num_embeddings > cardinality_threshold + 1:
                ...
                count += 1
        logging.info(f"PCVRFusionFormer3V2: re-initialized {count} high-cardinality embeddings")
        return reinit_ptrs


# infer.py dynamic import requires PCVRHyFormer alias
PCVRHyFormer = PCVRFusionFormer3V2
