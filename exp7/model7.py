"""exp7/model7.py — PCVRFusionFormer7

Changes from model6.py (CrossDomainInterestTransfer only):
  A. __init__(): num_domains parameter added, self.temperature (nn.Parameter) added
     init value = sqrt(d_model) * 0.1 ≈ 1.131 for d_model=128
     (much smaller than exp6 fixed scale sqrt(128)≈11.3 → initially peaked attention)
  B. forward(): enumerate(domains) + per-domain temperature clamp replaces fixed scale
  C. _tokenize(): TB logs temperature/{domain} added (4 scalars)

Everything else (EventEncoder, _score, _run_blocks, tokenizers,
UnifiedInteractionBlock, forward, predict) is identical to model6.py.

Alias:
  PCVRHyFormer = PCVRFusionFormer7  (for infer.py dynamic import)
"""
...
class CrossDomainInterestTransfer(nn.Module):
    ...
    Step A: Target-aware pooling — item_repr × learnable_temp × decay_weights × attn_weights
    Step B: Cross-domain MHA — domain representations attend to each other

    Change from model6: fixed scale=sqrt(D) replaced by per-domain learnable temperature.
    Init: temperature[d] = sqrt(d_model) * 0.1 ≈ 1.131  (d_model=128)
    This is 10× smaller than exp6's fixed scale, yielding peaked attention from step 1.
    """
    ...
    def __init__(
        ...
        # Per-domain learnable temperature; init = sqrt(d_model) * 0.1
        self.temperature = nn.Parameter(
            torch.ones(num_domains) * (d_model ** 0.5) * 0.1
        )
    ...
    def forward(
        ...
        for d_idx, domain in enumerate(domains):
            ...
            # Dot-product attention with learnable temperature (clamp avoids collapse to 0)
            t = torch.clamp(self.temperature[d_idx], min=0.01)
            scores = torch.bmm(embs, item_repr.unsqueeze(-1)).squeeze(-1) / t  # (B, L)
...
class PCVRFusionFormer7(nn.Module):
    """PCVRFusionFormer7: identical to model6 except CrossDomainInterestTransfer.

    CrossDomainInterestTransfer change:
      - Adds learnable per-domain temperature (nn.Parameter, shape=[num_domains])
      - Init: sqrt(d_model) * 0.1 — 10× smaller than exp6 fixed scale
      - Replaces fixed / sqrt(D) division in target attention score computation
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ── Cross-Domain Interest Transfer (with learnable temperature) ───────
        ...
        # Signal 1: ui_mlp — cat([ui, item, ui-item, ui*item]) → scalar
        ...
        logging.info(
            f"PCVRFusionFormer7: U={self.num_user_ns} I={self.num_item_ns} "
    ...
    def _tokenize(self, inputs: ModelInput, training: bool) -> torch.Tensor:
        ...
        if self._tb_logs is not None:
            with torch.no_grad():
                ca = cross_attn_w.mean(0)  # (num_domains, num_domains)
                ...
                for d_idx, domain in enumerate(self.seq_domains):
                    t = torch.clamp(self.cross_domain_transfer.temperature[d_idx], min=0.01)
                    self._tb_logs[f'temperature/{domain}'] = t.item()
    ...
    def _score(self, all_tok: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        gates = torch.sigmoid(self.gate_linear(gate_in))                    # (B, 3)
    ...
    def reinit_high_cardinality_params(self, cardinality_threshold: int = 10000) -> "set[int]":
        ...
        for m in self.modules():
            if isinstance(m, nn.Embedding) and m.num_embeddings > cardinality_threshold + 1:
                ...
                count += 1
        logging.info(f"PCVRFusionFormer7: re-initialized {count} high-cardinality embeddings")
        return reinit_ptrs


# infer.py dynamic import requires PCVRHyFormer alias
PCVRHyFormer = PCVRFusionFormer7
