...
def main() -> None:
    ...
    # ---- Schema: prefer the one from model_dir; use train_config schema basename ----
    #      as a second candidate before falling back to data_dir.
    #      trainer.py copies the schema file with its original basename (e.g.
    #      schema_aligned.json) so we cannot assume the name is always schema.json.
    schema_path = os.path.join(model_dir, 'schema.json')
    if not os.path.exists(schema_path):
        _cfg_schema = train_config.get('schema_path', '')
        if _cfg_schema:
            _local = os.path.join(model_dir, os.path.basename(_cfg_schema))
    ...
    # ---- Dynamic import: read dataset_module / model_module from train_config ----
    _dataset_module = train_config.get('dataset_module', 'dataset')
    _model_module = train_config.get('model_module', 'model')

    if _dataset_module != 'dataset' or _model_module != 'model':
        _script_dir = os.path.dirname(os.path.abspath(__file__))
        _exp_dir = os.path.normpath(os.path.join(_script_dir, '..', 'exp'))
        if os.path.isdir(_exp_dir) and _exp_dir not in sys.path:
            sys.path.insert(0, _exp_dir)
            logging.info(f"Added exp/ to sys.path: {_exp_dir}")

    if _dataset_module != 'dataset':
        _ds = importlib.import_module(_dataset_module)
        global FeatureSchema, PCVRParquetDataset, NUM_TIME_BUCKETS
        FeatureSchema = _ds.FeatureSchema
        PCVRParquetDataset = _ds.PCVRParquetDataset
        NUM_TIME_BUCKETS = _ds.NUM_TIME_BUCKETS
        _FALLBACK_MODEL_CFG['num_time_buckets'] = NUM_TIME_BUCKETS
        logging.info(f"Dynamic dataset import: {_dataset_module} → {PCVRParquetDataset.__name__}")

    if _model_module != 'model':
        _model_mod = importlib.import_module(_model_module)
        global PCVRHyFormer
        PCVRHyFormer = _model_mod.PCVRHyFormer
        logging.info(f"Dynamic model import: {_model_module} → {PCVRHyFormer.__name__}")
