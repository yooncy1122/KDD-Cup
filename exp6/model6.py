"""exp6/model6.py — PCVRFusionFormer6
...
Alias:
  PCVRHyFormer = PCVRFusionFormer6  (for infer.py dynamic import)
"""
...
class PCVRFusionFormer6(nn.Module):
    """PCVRFusionFormer6: identical to model5 except _score().
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logging.info(
            f"PCVRFusionFormer6: U={self.num_user_ns} I={self.num_item_ns} "
    ...
    def reinit_high_cardinality_params(self, cardinality_threshold: int = 10000) -> "set[int]":
        ...
        for m in self.modules():
            if isinstance(m, nn.Embedding) and m.num_embeddings > cardinality_threshold + 1:
                ...
                count += 1
        logging.info(f"PCVRFusionFormer6: re-initialized {count} high-cardinality embeddings")
        return reinit_ptrs


# infer.py dynamic import requires PCVRHyFormer alias
PCVRHyFormer = PCVRFusionFormer6
