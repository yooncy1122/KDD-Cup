"""exp2/model2.py — PCVRFusionFormer
...
Architecture:
  ...
  ScoringHead              : user-item cross-attn + domain similarity → MLP → logit

Alias:
  PCVRHyFormer = PCVRFusionFormer  (for infer.py dynamic import)
"""
...
class StructuredUserNSTokenizer(nn.Module):
    ...
    def _weighted_emb(
        ...
        """Softmax-weighted embedding sum for an aligned pair feature.

        Args:
            int_vals : (B, L) integer bucket indices
            coeff    : (B, L) float coefficients from user_dense
        Returns:
            (B, emb_dim)
        """
    ...
    def forward(
        ...
        """
        Args:
            user_int_feats   : (B, 234)
            user_dense_feats : (B, 785) or (B, 755) — recon fallback
        Returns:
...
class TimeDecayEncoder(nn.Module):
    ...
    Larger time_bucket → older inter-event gap → lower weight.
    """
    ...
    def forward(
        ...
        # Pooling score = content + time-decay
...
class UnifiedInteractionBlock(nn.Module):
    ...
    No d_model % T divisibility constraint — uses standard nn.MultiheadAttention.
    NS↔S and User↔Item interaction are implicit in full self-attention.
    """
...
class PCVRFusionFormer(nn.Module):
    """PCVRFusionFormer: structured NS tokenization + time-decay sequence encoding
    + unified full-attention interaction + user-item cross-attn scoring.
    """
    ...
    def __init__(
        ...
        # absorb all other train.py flags (num_queries, seq_encoder_type, etc.)
        **kwargs,
    ) -> None:
        super().__init__()
        self.d_model = d_model
        ...
        logging.info(
            f"PCVRFusionFormer: U={self.num_user_ns} I={self.num_item_ns} "
    ...
    def _score(self, all_tok: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        logits = self.classifier(features)                                    # (B, action_num)
    ...
    def reinit_high_cardinality_params(self, cardinality_threshold: int = 10000) -> "set[int]":
        ...
        for m in self.modules():
            if isinstance(m, nn.Embedding) and m.num_embeddings > cardinality_threshold + 1:
                ...
                count += 1
        logging.info(f"PCVRFusionFormer: re-initialized {count} high-cardinality embeddings")
        return reinit_ptrs


# infer.py dynamic import requires PCVRHyFormer alias
PCVRHyFormer = PCVRFusionFormer
