"""exp5/model5.py — PCVRFusionFormer5
...
Architecture:
  ...
  EventEncoder × 4             : event-level embed → (event_embs, decay_weights), NO pooling
  CrossDomainInterestTransfer  : target-aware domain pooling (multiplicative attn×decay)
                                 + cross-domain MHA → domain repr tokens
  UnifiedInteractionBlock × L  : full self-attention over all tokens (NS+S)
  GatedScoringHead             : ui_score + domain_sim + rich_repr → gated logit

Alias:
  PCVRHyFormer = PCVRFusionFormer5  (for infer.py dynamic import)
"""
...
class CrossDomainInterestTransfer(nn.Module):
    ...
    def forward(
        ...
        Args:
            item_repr         : (B, d_model)
            event_embs_dict   : {domain: (B, L, d_model)}
            decay_weights_dict: {domain: (B, L)} — exp(-λ·t/63), padded=0
            seq_lens_dict     : {domain: (B,)}
            domains           : ordered list of domain names
        Returns:
            cross_domain_out  : (B, num_domains, d_model)
            cross_attn_weights: (B, num_domains, num_domains)
            attn_w_list       : List[(B, L)] per domain — target attention weights
        """
        ...
        )  # (B, D, D), (B, num_domains, num_domains)
...
class PCVRFusionFormer5(nn.Module):
    """PCVRFusionFormer5: structured NS tokenization + event encoding
    + cross-domain interest transfer + unified attention + gated scoring.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Gate: input = cat([ui_score(1), domain_sim(S), rich_scalar(1)]) = 1+S+1
        ...
        logging.info(
            f"PCVRFusionFormer5: U={self.num_user_ns} I={self.num_item_ns} "
    ...
    def _tokenize(self, inputs: ModelInput, training: bool) -> torch.Tensor:
        ...
        if self._tb_logs is not None:
            with torch.no_grad():
                # Cross-domain attention matrix (batch mean) — 4×4 = 16 scalars
                ca = cross_attn_w.mean(0)  # (4, 4)
                for i in range(ca.shape[0]):
                    for j in range(ca.shape[1]):
                        self._tb_logs[f'xdomain/attn_{i}{j}'] = ca[i, j].item()
                # Per-domain target attention entropy
    ...
    def _score(self, all_tok: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Signal 1: element-wise dot product compatibility
        ui_score = (ui_repr * item_repr).sum(dim=-1, keepdim=True) / math.sqrt(self.d_model)  # (B, 1)
    ...
    def reinit_high_cardinality_params(self, cardinality_threshold: int = 10000) -> "set[int]":
        ...
        for m in self.modules():
            if isinstance(m, nn.Embedding) and m.num_embeddings > cardinality_threshold + 1:
                ...
                count += 1
        logging.info(f"PCVRFusionFormer5: re-initialized {count} high-cardinality embeddings")
        return reinit_ptrs


# infer.py dynamic import requires PCVRHyFormer alias
PCVRHyFormer = PCVRFusionFormer5
