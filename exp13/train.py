...
from dataset import FeatureSchema, get_pcvr_data, NUM_TIME_BUCKETS  # default; overridden by --dataset_module
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--sim_top_k', type=int, default=50,
                        help='SIM hard search top-K events per domain '
                             '(0 = disabled, use all events)')
    parser.add_argument('--window_size', type=int, default=64,
                        help='Sliding window size for top BFTS blocks '
                             '(0 = full attention, same as bottom blocks)')
    ...
    parser.add_argument('--dataset_module', type=str, default='dataset',
                        help='Dataset module name to import (default: dataset)')
    parser.add_argument('--model_module', type=str, default='model',
                        help='Model module to import PCVRHyFormer from (default: model)')
...
def main() -> None:
    ...
    # Override dataset symbols if a non-default module is requested.
    if args.dataset_module != 'dataset':
        import importlib
        _ds = importlib.import_module(args.dataset_module)
        global FeatureSchema, get_pcvr_data, NUM_TIME_BUCKETS
        FeatureSchema = _ds.FeatureSchema
        get_pcvr_data = _ds.get_pcvr_data
        NUM_TIME_BUCKETS = _ds.NUM_TIME_BUCKETS

    # Override model class if a non-default module is requested.
    if args.model_module != 'model':
        import importlib
        _model_mod = importlib.import_module(args.model_module)
        global PCVRHyFormer
        PCVRHyFormer = _model_mod.PCVRHyFormer
    ...
    # Inject aligned-pair dense plan when using model_weighted.
    if args.model_module == 'model_weighted':
        _WEIGHTED_FIDS = [62, 63, 64, 65, 66]
        _int_fids = [fid for fid, _, _ in pcvr_dataset.user_int_schema.entries]
        _user_dense_aligned_plan = []
        for _fid in _WEIGHTED_FIDS:
            if _fid in _int_fids and _fid in pcvr_dataset.user_dense_schema._fid_to_entry:
                _fid_idx = _int_fids.index(_fid)
                _dense_off, _dense_len = pcvr_dataset.user_dense_schema.get_offset_length(_fid)
                _user_dense_aligned_plan.append((_fid_idx, _dense_off, _dense_len))
        model_args['user_dense_aligned_plan'] = _user_dense_aligned_plan
        logging.info(f"user_dense_aligned_plan (fid_idx, dense_off, dense_len): {_user_dense_aligned_plan}")
