"""exp8/model8.py — PCVRFusionFormer8

Changes from model5.py (_score() only):
  A. ui_score: dot-product → MLP(cat([ui, item, ui-item, ui*item]))
  B. gate input: LayerNorm before sigmoid gate
...
Alias:
  PCVRHyFormer = PCVRFusionFormer8  (for infer.py dynamic import)
"""
...
class PCVRFusionFormer8(nn.Module):
    """PCVRFusionFormer8: identical to model6 (same architecture, class rename only).
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Signal 1 (A): ui_mlp — replaces dot-product ui_score
        ...
        logging.info(
            f"PCVRFusionFormer8: U={self.num_user_ns} I={self.num_item_ns} "
    ...
    def reinit_high_cardinality_params(self, cardinality_threshold: int = 10000) -> "set[int]":
        ...
        for m in self.modules():
            if isinstance(m, nn.Embedding) and m.num_embeddings > cardinality_threshold + 1:
                ...
                count += 1
        logging.info(f"PCVRFusionFormer8: re-initialized {count} high-cardinality embeddings")
        return reinit_ptrs


# infer.py dynamic import requires PCVRHyFormer alias
PCVRHyFormer = PCVRFusionFormer8
