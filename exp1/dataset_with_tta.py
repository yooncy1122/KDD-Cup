...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        _TIME_TO_ACTION_FID = 110
        self._time_to_action_offset: Optional[int] = None
        self._user_dense_plan = []
        offset = 0
        for fid, dim in self._user_dense_cols:
            ci = self._col_idx.get(f'user_dense_feats_{fid}')
            if fid == _TIME_TO_ACTION_FID:
                self._time_to_action_offset = offset  # parquet 컬럼 없음, plan에서 제외
            else:
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        # ---- time_to_action = label_time - timestamp (training only) ----
        if self._time_to_action_offset is not None:
            if self.is_training and 'label_time' in self._col_idx:
                label_times = (batch.column(self._col_idx['label_time'])
                               .fill_null(0).to_numpy(zero_copy_only=False)
                               .astype(np.int64))
                time_to_action = np.maximum(label_times - timestamps, 0).astype(np.float32)
                user_dense[:B, self._time_to_action_offset] = time_to_action
            # is_training=False 또는 label_time 없음: user_dense[:] = 0으로 이미 처리됨
